<template>
  <div
    ...
    :class="where == 'deadline' ? 'h-36 shadow-md w-1/3 mr-3' : ' h-32 md:h-40 shadow-lg'"
  >
    ...
    <div class="w-4/6 truncate block pl-4 pt-3">
      ...
      <div v-if="type.startsWith('homework')" class="flex w-32 pl-8 mt-1 mx-auto" style="color:#6e6e6e">
      ...
      <div v-if="!done && type.startsWith('homework')" class="text-gray-500 w-32" :class="where == 'deadline' ? '-mt-2 -ml-1' : 'mt-2'">
      {{
        Math.ceil(
...
export default {
  ...
  methods: {
    zoomWork(){
      this.$store.commit("setWorkUI", {id: this.id, type: this.type, owner: this.owner, title: this.title, subtitle: this.subtitle, makers: this.makers, shared: this.shared, finish: this.shared, image: this.image, comments: this.comments})
    }
  },
  created(){
