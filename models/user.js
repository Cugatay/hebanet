...
async function getUser(username, password, why, token) {
  ...
  return new Promise(async (res, rej) => {
    db.all(
      why == "login"
        ? `SELECT * FROM users WHERE username = '${username}'` : why == "userpage" ? `SELECT name,description,role,teachers,friends,students,username,work_area FROM users WHERE username = '${username}'` : why == "getstudents" ? `SELECT name,role,username,description,work_area FROM users WHERE teachers LIKE '%"username":"${username}"%'` : null,
      function(err, rows) {
        ...
        if(why == 'userpage'){
          ...
          res({ err: "Kullanıcı adı hatalı" })
        }
        else if(why == 'login' && password != undefined && rows[0] != undefined){

          if(token == undefined){
          bcrypt.compare(password, rows[0].password, (err, validate) => {
            if(validate){
            const newtoken = jwt.sign({username: rows[0].username, password: rows[0].password}, secret_key, { expiresIn: "60d" });
...
function searchUser(name) {
  return new Promise((res, rej) => {
    ...
    db.all(
      ...
      db.all(`SELECT name,role,username,description,work_area FROM users WHERE username LIKE '%${name}%'`, function(err2, rows2){
  ...
  return new Promise(async (res, rej) => {
        ...
        if (user.err != undefined) {
          bcrypt.hash(password, 9, (err, cryptopass) => {
            ...
            const newtoken = jwt.sign({username: username, password: cryptopass}, secret_key, { expiresIn: "60d" });
...
function addUser(stateUsername, statePassword, targetUsername, token) {
  return new Promise(async (res, rej) => {
        ...
        if(user.err == undefined){
              ...
              let targetUserSlim = `{"name": "${targetUser.name}", ${targetUser.description != undefined ? `"description": "${targetUser.description}",` : ""} "role": "${targetUser.role}", "username": "${targetUser.username}" ${targetUser.work_area != undefined ? `,"work_area": "${targetUser.work_area}"` : ""}  }`
            ...
            targetUser = JSON.parse(targetUserSlim)
                ...
                let addedFriendsBool = addedFriends[0] != undefined ? addedFriends.find((friend) => {
                    return friend.username == targetUser.username
                }) : undefined;
                
                let addedTeachers = JSON.parse(`[${user.teachers}]`)
                let addedTeachersBool = addedTeachers[0] != undefined ? addedTeachers.find((teacher) => {
                    return teacher.username == targetUser.username
                }) : undefined

                if (targetUser.err == undefined && addedFriendsBool == undefined && addedTeachersBool == undefined) {
                  if (targetUser.role == user.role) {                   //--------------------------------------------------  ÇAPULCU EKLERSİN --------------------------------------------------------
                    let adduser = user.friends != undefined && user.friends != "" ? user.friends + "," + JSON.stringify(targetUser) : JSON.stringify(targetUser);
                  ...
                  else if(targetUser.role == 'teacher' && (user.role == 'student' || user.role == 'Çapulcu')){
                    let adduser = user.teachers != undefined && user.teachers != "" ? user.teachers + "," + JSON.stringify(targetUser) : JSON.stringify(targetUser);
                    ...
                    res({ err: "Öğretmen, öğrencilerini herhangi bir şekilde ekleyemez" })
                  }
                } else {
...
function removeUser(stateUsername, statePassword, targetUsername, token){
    return new Promise(async (res, rej) => {
        ...
        console.log(user)
            ...
            let findFriend = addedFriends[0] != undefined ? addedFriends.find(friend => {
                return friend.username == targetUsername;
            }) : undefined

            let findTeacher = addedTeachers[0] != undefined ? addedTeachers.find(teacher => {
            ...
            addedFriends.forEach(friend => {
              console.log(friend)
                ...
                resObj += resObj != "" ? `,${JSON.stringify(friend)}` : `${JSON.stringify(friend)}`
            });
            console.log('object')

            db.all("UPDATE users SET friends = (?) WHERE username = (?) AND password = (?)", [resObj, user.username, user.password], function(err, rows){
                if(err) console.log(err)
                ...
                resObj += resObj != "" ? `,${JSON.stringify(teacher)}` : `${JSON.stringify(teacher)}`
                    }
                })
                 db.all("UPDATE users SET teachers = ? WHERE username = ? AND password = ?", [resObj, user.username, user.password], function(err, rows){
                if(err) console.log(err)
